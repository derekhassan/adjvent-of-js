...
const episodeCheckboxes = document.querySelectorAll(
    '.episodes input[type="checkbox"]'
);

const episodesListItems = document.querySelectorAll('.episodes li span');

let lastChecked = null;

const onChange = (e) => {
    const checkbox = e.target.parentNode.firstElementChild;
    const currentId = parseInt(checkbox.id.slice(-1));
    if (e.shiftKey) {
        firstIndex = currentId < lastChecked ? currentId - 1 : lastChecked;
        secondIndex = currentId < lastChecked ? lastChecked : currentId;

        episodes.slice(firstIndex, secondIndex).map(({ id }) => {
            const checkbox = document.querySelector(`#episode-${id}`);
            if (checkbox) {
                checkbox.checked = true;
            }
        });
    }
    lastChecked = parseInt(checkbox.id.slice(-1));
};
...
episodesListItems.forEach((checkbox) =>
